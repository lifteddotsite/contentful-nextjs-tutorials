...
export default function JobsPage({ jobs }) {
  ...
  return (
    <div className='flex flex-col space-y-10 sm:flex-row sm:space-x-6 sm:space-y-0 md:flex-col md:space-x-0 md:space-y-10 xl:flex-row xl:space-x-6 xl:space-y-0 mt-9'>
      <JobsPageSideBarForm setdisplayedJobs={setDisplayedJobs} />
      <div className='w-full'>
        <SearchJobForm setdisplayedJobs={setDisplayedJobs} />
        {/* Jobs header */}
        <div className='flex justify-between items-center mb-4'>
          <div className='text-sm text-slate-500 italic'>{title}</div>
          {/* Sort */}
          <JobsSortForm jobs={displayedJobs} setDisplayedJobs={setDisplayedJobs} />
        </div>
